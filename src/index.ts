    ...
    try {
      ...
      // OpenClaw API routes
        ...
        if (request.method === 'POST') {
          const { userId, taskType, payload } = await request.json() as { userId: string; taskType: 'scraping' | 'form_filling' | 'scheduling'; payload: unknown };
          response = await openClawAPI.createTask(userId, taskType, payload);
          
          // Record usage for billing
          const result = await response.clone().json() as { taskId: string };
          if (result.taskId) {
      ...
      // Q-Emplois routes (high-risk AI)
        ...
        if (request.method === 'POST') {
          const { userId, taskType, payload } = await request.json() as { userId: string; taskType: 'candidate_screening' | 'job_matching' | 'skill_assessment'; payload: unknown };
        ...
        const { oversightId, reviewerId, decision, reasoning } = await request.json() as { oversightId: string; reviewerId: string; decision: 'approved' | 'rejected'; reasoning: string };
      ...
      // Zyeuté Content routes
        ...
        if (request.method === 'POST') {
          const { userId, taskType, payload } = await request.json() as { userId: string; taskType: 'rss_scrape' | 'ai_summarize' | 'affiliate_inject'; payload: unknown };
        ...
        const { userId, feedUrl, affiliateLinks } = await request.json() as { userId: string; feedUrl: string; affiliateLinks: Array<{ keyword: string; url: string }> };
...
async function handleUserRegistration(
  ...
  try {
    ...
    await env.DB
      .prepare(
        'INSERT INTO users (id, email, created_at, region, subscription_tier) VALUES (?, ?, ?, ?, ?)'
      )
