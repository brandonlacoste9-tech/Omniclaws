...
export class OpenClawAPI {
    constructor(env) {
        this.TASK_PRICE = 0.05; // $0.05 per task
        this.db = env.DB;
     ...
     */
    async createTask(userId, taskType, payload) {
        try {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
                .bind(taskId, userId, 'openclaw', taskType, 'pending', JSON.stringify(payload), Date.now(), 0)
                .run();
            // Log task creation
     ...
     */
    async performFormFilling(payload) {
        try {
     ...
     */
    async performScheduling(payload) {
