...
export class QEmploisService {
     ...
     */
    async createRecruitmentTask(userId, taskType, payload) {
        try {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
                .bind(taskId, userId, 'q-emplois', taskType, 'pending', JSON.stringify(payload), Date.now(), 0)
                .run();
    ...
    async executeTask(taskId) {
        try {
            ...
            const evaluation = await this.aiActMonitor.evaluateDecision(task.user_id, taskId, 'q-emplois', aiDecision);
     ...
     */
    async submitReview(oversightId, reviewerId, decision, reasoning) {
        try {
